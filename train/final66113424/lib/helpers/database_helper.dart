...
/// รีเซ็ตฐานข้อมูล (ลบข้อมูลเก่าและลงข้อมูลเริ่มต้นใหม่)
Future<void> resetDatabase() async {
  final db = await database;
  
  // ลบตารางเก่า
  await db.execute("DROP TABLE IF EXISTS ${SqlCommands.incidentReport}");
  await db.execute("DROP TABLE IF EXISTS ${SqlCommands.violationType}");
  await db.execute("DROP TABLE IF EXISTS ${SqlCommands.pollingStation}");
...
/// ดึงรายชื่อเขตทั้งหมด (ไม่ซ้ำ) เรียงตามชื่อ
